...
const WebDateTimeInput = ({ value, onChange }: any) => {
...
const PrivateCarBookingScreen = ({ navigation }: any) => {
  ...
  useEffect(() => {
    privateCarApi.getAll()
      .then(res => setCars(res.data))
      .catch(() => setError('Failed to load cars.'))
      .finally(() => setLoading(false));
  }, []);

  // Price = price_per_km × estimated distance
  const km = Number(estimatedKm) || 10;
  const totalPrice = selectedCar
    ? (Number(selectedCar.price_per_km) * km).toLocaleString()
    : '0';
  ...
  const handleBook = async () => {
    ...
    try {
      ...
      setSuccess(`✅ Booked! ${selectedCar.driver_name} will pick you up at ${pickup}.`);
      setTimeout(() => {
          ...
          {cars.length === 0 ? (
            ...
            cars.map((car) => (
              <TouchableOpacity
                ...
                activeOpacity={0.9}
                onPress={() => { setSelectedCar(car); setShowBookingModal(true); setError(''); setSuccess(''); setPickup(''); setDropoff(''); setPickupTime(''); setPaymentMethod(null); }}
              >
                ...
                <View className="flex-row justify-between items-center pt-3 border-t border-gray-100">
                  <View>
                    ...
                    <Text className="text-xl font-bold" style={{ color: colors.primary }}>
                      {Number(car.price_per_km).toLocaleString()} RWF/km
                    </Text>
                  </View>
                  <TouchableOpacity
            ...
            <ScrollView contentContainerStyle={{ padding: 24, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
              ...
              <View className="flex-row justify-between items-center mb-6">
                <View>
                  ...
                  <Text className="text-xs" style={{ color: colors.textSecondary }}>Driver: {selectedCar?.driver_name} • {selectedCar?.plate}</Text>
                </View>
              ...
              <View className="mb-6">
                ...
                <View className="flex-row items-center border rounded-xl px-3 py-3" style={{ borderColor: colors.primary }}>
                  ...
                  <View style={{ flex: 1, marginLeft: 8 }}>
                    <WebDateTimeInput value={pickupTime} onChange={setPickupTime} />
                  </View>
                </View>
              </View>
              ...
              {/* Distance estimator */}
              <View className="mb-4">
                <Text className="text-xs font-bold text-gray-400 uppercase mb-2">Estimated Distance (km)</Text>
                <View className="flex-row items-center border rounded-xl px-3 py-3" style={{ borderColor: colors.primary }}>
                  <TextInput
                    value={estimatedKm}
                  ...
                  />
                  <Text style={{ color: colors.textSecondary, fontSize: 12 }}>km</Text>
                </View>
              </View>
              ...
              <View className="bg-gray-50 rounded-2xl p-4 mb-6 border border-gray-100">
                <View className="flex-row justify-between mb-1">
                  <Text className="text-sm" style={{ color: colors.textSecondary }}>Rate</Text>
                  <Text className="text-sm" style={{ color: colors.textSecondary }}>{Number(selectedCar?.price_per_km).toLocaleString()} RWF/km</Text>
                </View>
                <View className="flex-row justify-between mb-1">
                  <Text className="text-sm" style={{ color: colors.textSecondary }}>Distance</Text>
                  <Text className="text-sm" style={{ color: colors.textSecondary }}>{estimatedKm || 10} km</Text>
                </View>
                <View className="flex-row justify-between pt-2 border-t border-gray-200 mt-1">
                  <Text className="text-sm font-bold" style={{ color: colors.text }}>Total</Text>
                  <Text className="text-base font-bold" style={{ color: colors.primary }}>{totalPrice} RWF</Text>
                </View>
              </View>
              ...
              {[
                ...
                <TouchableOpacity
                  ...
                  style={{ backgroundColor: paymentMethod === method.id ? colors.primary + '10' : '#F9FAFB', borderColor: paymentMethod === method.id ? colors.primary : '#E5E7EB' }}
                >
